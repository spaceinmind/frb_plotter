...
if np.sum(rfi_flag_original) > 0:
    ...
    flagged_freqs = freqs[rfi_flag_original]
...
# Calculate robust statistics using valid channels (exclude manually-flagged ones)
...
if np.sum(valid_channels) > 10:
    ...
    # Don't apply the low-median check to avoid re-flagging our manually-zeroed channels
    
    # If too many AUTO-flagged channels (>50% beyond manual flags), skip auto-flagging
    n_auto = np.sum(rfi_flag & ~rfi_flag_original)
    if n_auto > len(freqs) * 0.5:
        print(f"  Warning: auto-RFI flagging would remove {n_auto}/{len(freqs)} extra channels (>{50}%)")
        print(f"  Skipping auto-RFI flagging to preserve data")
        rfi_flag = rfi_flag_original.copy()
else:
    # Not enough valid channels, skip auto-RFI flagging
    rfi_flag = rfi_flag_original.copy()
...
# Build title: line 1 = name + telescope, line 2 = observing/pulse params
title_line1 = base_name
if telescope:
    title_line1 += f" ({telescope})"
